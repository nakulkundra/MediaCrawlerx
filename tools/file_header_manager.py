...
DISCLAIMER = """# Disclaimer: This code is for educational and research purposes only. Users must adhere to the following principles:
# 1. Do not use for any commercial purposes.
# 2. Comply with the target platform's Terms of Service and robots.txt rules during use.
# 3. Do not conduct large-scale scraping or cause operational disruptions to the platform.
# 4. Reasonably control request frequencies to avoid placing unnecessary burdens on target platforms.
...
def has_disclaimer(content: str) -> bool:
    ...
    """
    return "Disclaimer: This code is for educational and research purposes only" in content or "声明：本代码仅供学习和研究目的使用" in content
