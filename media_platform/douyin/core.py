...
class DouYinCrawler(AbstractCrawler):
    ...
    async def start(self) -> None:
        ...
        async with async_playwright() as playwright:
            ...
            if config.ENABLE_CDP_MODE:
                utils.logger.info("[DouYinCrawler] Launching browser using CDP mode")
                ...
                utils.logger.info("[DouYinCrawler] Launching browser using standard mode")
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while (page - start_page + 1) * dy_limit_count <= config.CRAWLER_MAX_NOTES_COUNT:
                ...
                if "data" not in posts_res:
                    utils.logger.error(f"[DouYinCrawler.search] search douyin keyword: {keyword} failed, account may be risk-controlled / rate-limited.")
                    break
    ...
    async def launch_browser_with_cdp(
        ...
        try:
            ...
            utils.logger.info(f"[DouYinCrawler] CDP browser info: {browser_info}")

            return browser_context

        except Exception as e:
            utils.logger.error(f"[DouYinCrawler] CDP mode launch failed, fallback to standard mode: {e}")
    ...
    async def get_aweme_media(self, aweme_item: Dict):
        """
        Get Douyin media, automatically determine whether the media type is short video or image note and download

        Args:
            aweme_item (Dict): Douyin item details
        """
