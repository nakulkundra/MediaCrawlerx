...
class KuaiShouClient(AbstractApiClient, ProxyRefreshMixin):
    ...
    async def request_rest_v2_signed(self, uri: str, data: dict) -> Dict:
        """
        REST API V2 request with __NS_hxfalcon signature
        Kuaishou web batch list endpoints (posts list/search) require signatures; unsigned requests return result:50
        Exponential backoff retry on server rate limiting (result:2)
        :param uri: API endpoint path
        :param data: request body
        :return: response data
        """
        ...
        for attempt in range(max_retry):
            ...
            # Signature binds to request content and time window, must be regenerated on retry
    ...
    async def get_video_by_creater_v2(self, userId: str, pcursor: str = "") -> Dict:
        """
        Get user post list - REST signed version
        Web client migrated post list to /rest/v/profile/feed and requires signature
        :param userId: User ID
        :param pcursor: Pagination cursor
        :return: Top level structure {result, pcursor, feeds, ...}
        """
    ...
    async def search_info_by_keyword_v2(
        ...
        """
        Keyword search - REST signed version
        Web client migrated search to /rest/v/search/feed and requires signature
        :param keyword: Search keyword
        :param pcursor: Pagination cursor (numeric page number string)
        :param search_session_id: Search session ID
        :return: Top level structure {result, pcursor, feeds, searchSessionId, ...}
        """
        post_data = {
            "keyword": keyword,
            "pcursor": pcursor,
    ...
    async def get_video_all_comments(
        ...
        while pcursor != "no_more" and len(result) < max_count:
            ...
            # Add 1-3 seconds random jitter on top of fixed delay to reduce server rate limiting probability
    ...
    async def get_all_videos_by_creator(
        ...
        while pcursor != "no_more":
            ...
            # REST endpoint uses result field for business status, must explicitly validate,
            # otherwise blocked endpoint (result:50) will be silently treated as "no more videos"
