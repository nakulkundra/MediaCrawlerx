...
class BilibiliCrawler(AbstractCrawler):
    ...
    async def get_creator_details(self, creator_id: int, semaphore: asyncio.Semaphore):
        ...
        async with semaphore:
            ...
            # Educational version: Only keep the minimum fields required for dynamics (temporary in-memory use), do not persist creator profile data.
            creator_info: Dict = {
                "id": creator_id,
                "name": creator_unhandled_info.get("name"),
            }
            # Educational version: Do not crawl followers/followings list (personal information of other users) to prevent harassment.
