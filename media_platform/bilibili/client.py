...
class BilibiliClient(AbstractApiClient, ProxyRefreshMixin):
    ...
    async def pong(self) -> bool:
        ...
        try:
            ...
            if response.get("isLogin"):
                utils.logger.info("[BilibiliClient.pong] Use cache login state get web interface successfully!")
                ping_flag = True
    ...
    async def get_video_info(self, aid: Union[int, None] = None, bvid: Union[str, None] = None) -> Dict:
        """
        Bilibili web video detail api, choose one parameter between aid and bvid
        :param aid: Video aid
    ...
    async def get_video_play_url(self, aid: int, cid: int) -> Dict:
        """
        Bilibili web video play url api
        :param aid: Video aid
        :param cid: cid
        :return:
        """
